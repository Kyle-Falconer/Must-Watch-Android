...
public class BatchIngredientRepository {
    private final BatchIngredientDao batchIngredientDao;

    @Inject
    public BatchIngredientRepository(BatchIngredientDao batchIngredientDao) {
        this.batchIngredientDao = batchIngredientDao;
    }

    public void addBatchIngredients(List<BatchIngredient> ingredients) {
        Timber.d("Adding %s BatchIngredient objects to the db", ingredients.size());
        Observable.fromCallable(() -> batchIngredientDao.insertAll(ingredients))
